...
function sheetXml(rows: AnalysisRow[]): string {
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><cols>${widths}</cols><sheetData><row r="1">${headers}</row>${body}</sheetData><autoFilter ref="A1:P${rows.length + 1}"/><freezePanes><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></freezePanes></worksheet>`;
}
...
export function buildXlsx(rows: AnalysisRow[]): Uint8Array {
