...
function validServiceLabel(value: string): boolean {
  ...
  return label.length >= 7 && label.length <= 150 && words.length <= 20 && !genericServiceLabel.test(label)
    ...
    && !/^(?:более\s|бухгалтер|дополнительные услуги|завод\s|инженер\s|инструкция\s|история\s|компания\s|менеджер\s|методика\s|налог\s|онлайн калькулятор|особенности\s|профиль нашей|программа\s|производитель\s|технология\s|мы\s)/iu.test(label)
    ...
    && !/(?:для объекта\s*:|(?:^|\s)(?:г|х)\.\s*[А-ЯЁ]|для главного управления)/u.test(label)
...
function looksLikeServiceOffering(value: string): boolean {
  ...
  if (/^модульн[а-яё]*\s+.*(?:мастерск|шиномонтаж|мойк|пункт прокат)|^производство камер хранения$/iu.test(label)) return false;
  if (/\.\s*\d+\s*(?:этаж|лет|дн)/iu.test(label)) return false;
