...
export async function POST(request: Request) {
  ...
  const input: AnalysisInput = { projectUrl: String(body.projectUrl || "").trim(), description: String(body.description || "").trim(), region: String(body.region || "").trim() };
  if (!input.projectUrl || !input.description || !input.region) return Response.json({ error: "Заполните ссылку на сайт, описание и регион." }, { status: 400 });
  try {
