...
type CompetitorUspPattern = MarketItem & { competitorsList: string[]; examples: string[] };
type ProposedUsp = { statement: string; rationale: string; contrast?: string; requirement?: string };
...
function competitorSiteUrl(site: string): string | null {
  const value = site.trim();
  if (!value || value === "Сайт не указан") return null;
  try {
...
export default function Home() {
  ...
  async function loadHistory() {
    ...
    try {
      ...
      for (const item of combined) {
        if ((item.summary?.serviceCatalogVersion || 0) >= 17 && (item.summary?.uspAnalysisVersion || 0) >= 2 && item.summary?.proposedUsps?.length) {
  ...
  async function deleteSaved(item: SavedAnalysis) {
    if (!window.confirm(`Удалить анализ «${item.title}»? Отменить это действие будет нельзя.`)) return;
    setError("");
    ...
    try {
      ...
      if (currentAnalysisId === item.id) {
        setRows([]); setColumns(baseColumns); setSources([]); setSummary(null);
        setCurrentAnalysisId(""); setCurrentAnalysisTitle(""); setStatus("");
      }
      if (editingId === item.id) setEditingId(null);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Не удалось удалить анализ.");
  ...
  return <main className="app-shell clean-shell">
    ...
    {summary && <section className="panel summary-panel">
      <div className="section-kicker">Выводы по рынку</div><h2>Лидеры, покрытие и возможности</h2><p className="muted">{summary.methodology}</p>
      <div className="summary-grid"><div><h3>Лидеры</h3>{summary.leaders.map((leader) => { const siteUrl = competitorSiteUrl(leader.site); return <div className="summary-row" key={leader.site}>{siteUrl ? <a className="leader-link" href={siteUrl} target="_blank" rel="noreferrer">{leader.name}</a> : <b>{leader.name}</b>}<span>{leader.score} баллов · {leader.reasons.join(", ") || "подтверждённые факты"}</span></div>; })}</div><div><h3>Пробелы рынка</h3><ul>{summary.gaps.map((item) => <li key={item}>{item}</li>)}</ul><h3>Рекомендации</h3><ul>{summary.recommendations.map((item) => <li key={item}>{item}</li>)}</ul></div><div><h3>Цены и риски</h3><p>{summary.price.note}</p><ul>{summary.risks.map((item) => <li key={item}>{item}</li>)}</ul></div></div>
      {summary.competitorUsps && summary.competitorUsps.length > 0 && <div className="competitor-usps"><div className="service-catalog-head"><div><h3>УТП конкурентов</h3><p>Повторяющиеся обещания, которые уже заняты рынком и сами по себе не создают отстройку.</p></div><span>{summary.competitorUsps.length} паттернов</span></div><div className="usp-pattern-grid">{summary.competitorUsps.slice(0, 8).map((item) => <article className="usp-pattern-card" key={item.name}><b>{item.name}</b><span>{item.competitors} из {summary.price.total} конкурентов · {item.coverage}%</span><p>{item.competitorsList.slice(0, 3).join(", ")}</p></article>)}</div></div>}
      {summary.proposedUsps && summary.proposedUsps.length > 0 && <div className="proposed-usps"><div className="service-catalog-head"><div><h3>Новые УТП для проекта</h3><p>Гипотезы позиционирования на основе УТП конкурентов, свободных позиций и пробелов рынка.</p></div><span>{summary.proposedUsps.length} вариантов</span></div><div className="usp-list">{summary.proposedUsps.map((item, index) => <article className="usp-card" key={`${index}-${item.statement}`}><span>{index + 1}</span><div><b>{item.statement}</b><p>{item.rationale}</p>{item.contrast && <small><strong>Отстройка:</strong> {item.contrast}</small>}{item.requirement && <small><strong>Нужно обеспечить:</strong> {item.requirement}</small>}</div></article>)}</div></div>}
      {summary.serviceCatalog && <div className="service-catalog"><div className="service-catalog-head"><div><h3>Список услуг по теме</h3><p>H1 отдельных страниц услуг конкурентов, отфильтрованные по описанию проекта. Без товаров, кластеров и придуманных URL.</p></div><span>{summary.serviceCatalog.length} услуг</span></div>{summary.serviceCatalog.length ? <><div className="service-catalog-list">{visibleServices.map((item, index) => <div className="service-catalog-item" key={item.name}><span>{index + 1}</span><b>{item.name}</b></div>)}</div>{summary.serviceCatalog.length > 5 && <button className="show-table service-catalog-toggle" onClick={() => setServicesExpanded((value) => !value)} aria-expanded={servicesExpanded}>{servicesExpanded ? "Свернуть до 5 услуг" : `Показать ещё ${summary.serviceCatalog.length - 5} услуг`}</button>}</> : <p className="muted">На доступных страницах конкурентов релевантные теме услуги не подтверждены.</p>}</div>}
    </section>}
    <section id="history" className="panel history-panel"><div className="results-head"><div><div className="section-kicker">Архив</div><h2>Сохранённые анализы</h2></div><input className="history-search" aria-label="Поиск по архиву" placeholder="Найти анализ…" value={historyQuery} onChange={(event) => setHistoryQuery(event.target.value)} /></div>{filteredHistory.length === 0 ? <p className="muted">{history.length ? "Ничего не найдено." : "Запусков пока нет — создайте первый анализ выше."}</p> : <div className="history-list">{filteredHistory.map((item) => <article className="history-row" key={item.id}><div className="history-icon">{item.title.slice(0, 1).toUpperCase()}</div><div className="history-title">{editingId === item.id ? <div className="rename-form"><input aria-label="Новое название анализа" value={draftTitle} onChange={(event) => setDraftTitle(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void saveTitle(item); if (event.key === "Escape") setEditingId(null); }} /><button className="ghost" onClick={() => void saveTitle(item)}>Сохранить</button><button className="text-button" onClick={() => setEditingId(null)}>Отмена</button></div> : <><b>{item.title}</b><button className="rename-button" onClick={() => { setEditingId(item.id); setDraftTitle(item.title); }}>Переименовать</button></>}<span>{item.region || "Россия"} · {new Date(item.createdAt).toLocaleDateString("ru-RU", { day: "numeric", month: "long", year: "numeric" })} · {item.rows.length} компаний</span>{item.projectUrl && <small>{item.projectUrl}</small>}</div><div className="history-actions"><button className="ghost" onClick={() => openSaved(item)}>Открыть</button><button className="icon-button" aria-label={`Скачать Excel: ${item.title}`} title="Скачать Excel" onClick={() => void downloadData(item.rows, item.columns, item.summary)}>↓</button><button className="delete-button" disabled={deletingId === item.id} onClick={() => void deleteSaved(item)}>{deletingId === item.id ? "Удаляем…" : "Удалить"}</button></div></article>)}</div>}</section>
    </div></main>;
}
