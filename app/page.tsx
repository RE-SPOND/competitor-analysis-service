...
export default function Home() {
  ...
  async function loadHistory() {
    ...
    try {
      ...
      const enriched = await Promise.all(combined.map(async (item) => {
        if ((item.summary?.serviceCatalogVersion || 0) >= 4) return item;
        try {
          const response = await fetch("/api/summary", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ rows: item.rows, columns: item.columns, sources: item.sources, summary: item.summary, description: item.description || item.title }) });
          const data = await response.json();
  ...
  return <main className="app-shell clean-shell">
    ...
    {summary && <section className="panel summary-panel"><div className="section-kicker">Выводы по рынку</div><h2>Лидеры, покрытие и возможности</h2><p className="muted">{summary.methodology}</p><div className="summary-grid"><div><h3>Лидеры</h3>{summary.leaders.map((leader) => <div className="summary-row" key={leader.site}><b>{leader.name}</b><span>{leader.score} баллов · {leader.reasons.join(", ") || "подтверждённые факты"}</span></div>)}</div><div><h3>Пробелы рынка</h3><ul>{summary.gaps.map((item) => <li key={item}>{item}</li>)}</ul><h3>Рекомендации</h3><ul>{summary.recommendations.map((item) => <li key={item}>{item}</li>)}</ul></div><div><h3>Цены и риски</h3><p>{summary.price.note}</p><ul>{summary.risks.map((item) => <li key={item}>{item}</li>)}</ul></div></div>{summary.serviceCatalog && <div className="service-catalog"><div className="service-catalog-head"><div><h3>Список услуг по теме</h3><p>H1 отдельных страниц услуг конкурентов, отфильтрованные по описанию проекта. Без товаров, кластеров и придуманных URL.</p></div><span>{summary.serviceCatalog.length} услуг</span></div>{summary.serviceCatalog.length ? <><div className="service-catalog-list">{visibleServices.map((item, index) => <div className="service-catalog-item" key={item.name}><span>{index + 1}</span><b>{item.name}</b></div>)}</div>{summary.serviceCatalog.length > 5 && <button className="show-table service-catalog-toggle" onClick={() => setServicesExpanded((value) => !value)} aria-expanded={servicesExpanded}>{servicesExpanded ? "Свернуть до 5 услуг" : `Показать ещё ${summary.serviceCatalog.length - 5} услуг`}</button>}</> : <p className="muted">На доступных страницах конкурентов релевантные теме услуги не подтверждены.</p>}</div>}</section>}
