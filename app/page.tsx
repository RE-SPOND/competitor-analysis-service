...
export default function Home() {
  ...
  return <main className="app-shell clean-shell">
    ...
    {summary && <section className="panel summary-panel"><div className="section-kicker">Выводы по рынку</div><h2>Лидеры, покрытие и возможности</h2><p className="muted">{summary.methodology}</p><div className="summary-grid"><div><h3>Лидеры</h3>{summary.leaders.map((leader) => <div className="summary-row" key={leader.site}><b>{leader.name}</b><span>{leader.score} баллов · {leader.reasons.join(", ") || "подтверждённые факты"}</span></div>)}</div><div><h3>Кластеры услуг</h3>{(summary.serviceClusters || []).map((cluster) => <div className="summary-row" key={cluster.name}><b>{cluster.name}</b><span>{cluster.services.length} услуг</span></div>)}{!summary.serviceClusters?.length && summary.services.map((item) => <div className="summary-row" key={item.name}><b>{item.name}</b><span>{item.competitors} из {summary.price.total} · {item.coverage}%</span></div>)}</div><div><h3>Пробелы рынка</h3><ul>{summary.gaps.map((item) => <li key={item}>{item}</li>)}</ul><h3>Рекомендации</h3><ul>{summary.recommendations.map((item) => <li key={item}>{item}</li>)}</ul></div><div><h3>Цены и риски</h3><p>{summary.price.note}</p><ul>{summary.risks.map((item) => <li key={item}>{item}</li>)}</ul></div></div>{summary.serviceCatalog && <div className="service-catalog"><div className="service-catalog-head"><div><h3>Полный каталог услуг для SEO-структуры</h3><p>Собран из предложений всех конкурентов. URL — рабочая рекомендация для будущей структуры сайта.</p></div><span>{summary.serviceCatalog.length} услуг</span></div>{summary.serviceCatalog.length ? <div className="service-catalog-table"><div className="service-catalog-row service-catalog-labels"><span>Услуга</span><span>Кластер</span><span>Покрытие</span><span>Страница</span></div>{summary.serviceCatalog.map((item) => <div className="service-catalog-row" key={`${item.cluster}-${item.name}`}><div><b>{item.name}</b><small>{item.competitorsList.join(", ")}</small></div><span>{item.cluster}</span><span>{item.competitors} из {summary.price.total} · {item.coverage}%</span><code>{item.suggestedPage}</code></div>)}</div> : <p className="muted">Услуги не удалось подтвердить в собранных данных.</p>}</div>}</section>}
