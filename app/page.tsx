  ...
  async function handleAnalyze(event: FormEvent) {
    ...
    try {
      ...
      if (data.summary && nextRows.length > 0) {
        const saved: SavedAnalysis = { id: crypto.randomUUID(), projectUrl: form.projectUrl, region: form.region, createdAt: new Date().toISOString(), rows: nextRows, columns: nextColumns, sources: data.sources ?? [], summary: data.summary };
        setHistory((current) => {
